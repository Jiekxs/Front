...
export const FormValidation = create((data = {}) => {
    ...
    test('nombre', 'No debe contener números', () => {
        enforce(data.nombre).matches(/^\p{L}+(?:\s\p{L}+)*$/u, 'No debe contener números ni caracteres especiales');
    });
    ...
    test('rol', 'Válido: Cliente / Mecanico ', () => {
        const generoInput = data.rol.toLowerCase().replace(/^\w/, (c: string) => c.toUpperCase());
